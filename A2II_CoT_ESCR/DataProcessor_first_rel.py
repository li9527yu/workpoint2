...
class MyDataset(Data.Dataset):
    def __init__(self,args,data_dir,img_data_dir,emotion_data,relation_data,caption_data,tokenizer):
        ...
        self.caption_examples=self.creat_caption_examples(caption_data)
    ...
    def search_emotion(self,aspect,img):
        for item in self.emotion_examples:
            if aspect==item['aspect'] and img==item['image']:
                return item['text_emotion'],item['image_emotion']
        return None,None
    ...
    def creat_caption_examples(self,data_dir):
    ...
    def transform(self,line):
        ...
        image_emotion = value["image_emtoion"]
        ...
        # text_emotion,image_emotion=self.search_emotion(aspect,imgID)

        relation=self.search_relation(aspect,imgID)
        image_caption=self.caption_examples[imgID][3]
        ...
        #     inputs=f"{instruction_related} Sentence: {text}  Image Emotion: {image_emotion} Sentence Emotion: {text_clue} aspect: {aspect}  OPTIONS: -positive -neutral -negative output:"
        # elif relation=='3':
        ...
        # else:
        #     inputs=f"{instruction_related} Sentence: {text} Sentence Emotion: {text_clue}  aspect: {aspect}  OPTIONS: -positive -neutral -negative output:"
        # inputs=f"{instruction_related} Sentence: {text}  Image Emotion: {image_emotion} Sentence Emotion: {text_emotion} aspect: {aspect}  OPTIONS: -positive -neutral -negative output:"
        
        # inputs=f"{instruction_related} Sentence: {text}  Sentence Emotion: {text_emotion} aspect: {aspect}  OPTIONS: -positive -neutral -negative output:"
        ...
        # 使用银标相关性  inputs = "qea:  <image></image> caption: {} ".format(caption) + "sentence: {} aspect: {}.".format(text, aspect)
        if relation=="the semantic relevance is relevant, the emotional relevance is relevant" :
            input_relation='1'
            # inputs="qea: <image></image> sentence emotion: {} image emotion: {}".format(text_clue,image_emotion) + "sentence: {} aspect: {}.".format(text, aspect)
            # inputs=f"{instruction_related} Sentence: {text}  Image Emotion: {image_emotion} Sentence Emotion: {text_clue} aspect: {aspect}  OPTIONS: -positive -neutral -negative output:"
            ...
            # MDCA输入
            # inputs="qea: <image></image> sentence emotion: {} ".format(text_clue) + "sentence: {} aspect: {}.".format(text, aspect)
            # AECR数据集输入的最好组合
            # inputs=f"{instruction_related} Sentence: {text} Sentence Emotion: {text_clue}  aspect: {aspect}  OPTIONS: -positive -neutral -negative output:"
        else:
            input_relation='0'
        ...
        # output='<emotion>{}</emotion>'.format(sentiment)
        # 处理标签
        ...
        # 
        return input_ids,input_attention_mask,input_hidden_states,input_pooler_outputs,labels,int(output_labels),int(input_relation)
