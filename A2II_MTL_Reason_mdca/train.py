...
import wandb
from tool import warmup_linear,get_rel_data,parse_sequences,compute_metrics,rel_parse_sequences
...
def get_parser():
    ...
    parser.add_argument('--output_dir', default="/data/lzy1211/code/A2II/instructBLIP/results/A2II-MTL-Reason-mdca", type=str)
...
def evaluate(args,model,test_dataloader, logger):
    ...
    semantic_rel_results,emotion_rel_results={},{}
    model.eval()
    for batch in tqdm(test_dataloader, desc="Evaluating"):
            inputs = {k: v.to(args.device) for k, v in batch.items() if k not in ["senti_label","semantic_rel_label","emotion_rel_label"]}
            senti_label = batch["senti_label"]  
            semantic_rel_label = batch["semantic_rel_label"]
            emotion_rel_label = batch["emotion_rel_label"]
            with torch.no_grad():
                a_sequence, ea_sequence, iea_sequence= model.generate(**inputs)
    ...
    semantic_rel_preds,emotion_rel_preds= rel_parse_sequences(iea_pred_sequences)

    a_result = compute_metrics(a_preds, senti_labels)
    ea_result = compute_metrics(ea_preds, senti_labels)
    iea_result = compute_metrics(iea_preds, senti_labels)
    ...
    semantic_rel_results.update(semantic_rel_result)
    emotion_rel_results.update(emotion_rel_result)

    results = {'a_results': a_results, 'ea_results': ea_results, 'iea_results': iea_results, 'semantic_rel_results': semantic_rel_results, 'emotion_rel_results': emotion_rel_results }
    results['avg_results'] = {}
    ...
    results['avg_results']['precision'] = (results['a_results']['precision'] + results['ea_results']['precision'] +  results['iea_results']['precision']) / 3
    results['avg_results']['recall'] = (results['a_results']['recall'] + results['ea_results']['recall'] +  results['iea_results']['recall']) / 3
...
def train_multitask(
    ...
    for train_idx in trange(args.EPOCHS, desc="Epoch"):
        ...
        for step,data in enumerate(tqdm(train_dataloader,desc="Iteration")):
            ...
            loss = (1-args.lamda)* (ea_loss +  iea_loss )/ 2   +  args.lamda * a_loss    
            loss.backward()
        ...
        logger.info("  Batch size = %d", args.BATCH_SIZE)
        dev_result=evaluate(args,model,val_dataloader, logger)
        for key in sorted(dev_result.keys()):
                logger.info("  %s = %s", key, str(dev_result[key]))

        # 保存 模型
        # save model
        if dev_result['acc'] >= max_senti_acc:
            max_senti_acc=dev_result['acc']
...
def main(args):
    ...
    train_dataset=MyDataset(train_data,f'/data/lzy1211/code/A2II/instructBLIP/reason_data/data/{args.dataset}/train_cause.json',rel_data,tokenizer,max_seq_len= args.max_seq_length)
    val_dataset=MyDataset(val_data,f'/data/lzy1211/code/A2II/instructBLIP/reason_data/data/{args.dataset}/dev_cause.json',rel_data,tokenizer,max_seq_len= args.max_seq_length)
    test_dataset=MyDataset(test_data,f'/data/lzy1211/code/A2II/instructBLIP/reason_data/data/{args.dataset}/test_cause.json',rel_data,tokenizer,max_seq_len= args.max_seq_length)
    
    test_dataloader= Data.DataLoader(dataset=test_dataset,shuffle=True, batch_size=args.BATCH_SIZE,num_workers=0,collate_fn=collate_fn)
    ...
    # # train
    train_multitask(args,model,train_dataset,val_dataset,optimizer_t5,logger)
    ...
    result=evaluate(args,model,test_dataloader, logger)
