...
class MyFlanT5(nn.Module):
    ...
    def forward(self,input_ids,input_multi_ids,input_attention_mask,input_multi_attention_mask,input_hidden_states,relation,decoder_input_ids=None, decoder_attention_mask=None, labels=None):
        ...
        with torch.no_grad():
            ...
            # 对需要屏蔽的样本归零
            relation_query = projected * (1 - mask)  # relation=0的样本完全归零
            
            # 创建注意力掩码
            relation_query_attention_mask = (1 - mask).squeeze(-1).long().expand(-1, relation_query.size(1))  # [bs, seq_len]
        ...
        inputs_embeds = torch.cat([relation_query, relation_inputs_embeds.to(relation_query.device)], dim=1)  # [bs, 32+128,768]
        attention_mask = torch.cat(
            [relation_query_attention_mask, input_attention_mask.to(relation_query_attention_mask.device)], dim=1
        )
        ...
        final_attention_mask = torch.cat(
            [torch.ones((query.size(0), 1), dtype=torch.long, device=query.device), input_attention_mask], dim=1
        )
    ...
    def generate(self,input_ids,input_multi_ids,attention_mask,input_multi_attention_mask,input_hidden_states,relation,decoder_input_ids=None, decoder_attention_mask=None, labels=None):
        ...
        inputs_embeds = torch.cat([relation_query, relation_inputs_embeds.to(relation_query.device)], dim=1)  # [bs, 32+128,768]
        attention_mask = torch.cat(
            [relation_query_attention_mask, attention_mask.to(relation_query_attention_mask.device)], dim=1
        )
        ...
        # multi_predicted_labels=self.tokenizer.batch_decode(multi_outputs, skip_special_tokens=True, clean_up_tokenization_spaces=False)
        # fusion_weight = self.compute_fusion_weight(relation)  # shape [bs, 1]
        # fused_final = (1 - fusion_weight) * fused_repr + fusion_weight * mm_repr
        fused_final =   fused_repr + mm_repr  # [B, H]
        final_query = self.fused_projection(fused_final).unsqueeze(1)  # [B, 1, H]
