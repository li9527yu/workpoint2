...
from CoT_Model_fuse_withoutImgFeat import MyFlanT5
...
def get_parser():
    ...
    parser.add_argument('--output_dir', default="/data/lzy1211/code/A2II/instructBLIP/results/A2II-CoT-ESCR", type=str)
...
def evaluate(model,test_dataloader, logger):
    model.eval()
    pred_sequence,multi_pred_sequence,senti_labels=[],[],[]
    for batch in tqdm(test_dataloader, desc="Testing"):
        ...
        with torch.no_grad():
            ...
            # multi_pred_sequence.extend(multi_outpus)
    ...
    # logger.info("Relation Sentiment Evaluation Results: %s", senti_result)
    result = {     
        f'senti_acc':senti_result['acc'],    
        f'senti_f1':senti_result['f1'],  
   
    }   
    # multi_senti_preds=parse_sequences(multi_pred_sequence)
    # multi_senti_result = compute_metrics(multi_senti_preds, senti_labels,logger)
    # logger.info("Multi Sentiment Evaluation Results: %s", multi_senti_result)
    # avg_senti_acc = (senti_result['acc'] + multi_senti_result['acc']) / 2
    # avg_senti_f1 = (senti_result['f1'] + multi_senti_result['f1']) / 2

    # result = {     
    #     f'senti_acc':avg_senti_acc,    
    #     f'senti_f1':avg_senti_f1,  
...
def train(args,model,train_dataset,val_dataset,optimizer_t5,logger,):
    ...
    for train_idx in trange(int(args.EPOCHS), desc="Epoch"):
        ...
        # wandb.log(
        # {
        #     "val_acc": dev_result['senti_acc'],
        #     "val_macro_f1": dev_result['senti_f1'],
        # })
        # 保存 模型
        # save model
...
def main(args):
    ...
    relation_data='/data/lzy1211/code/A2II/instructBLIP/two_relation_inference_output/twitter2017/train_result.json'


    train_data=f"{args.data_dir}/train.json"
    train_img_data=f"{args.img_feat_dir}/twitter2017/train.pkl"
    
    val_data=f"{args.data_dir}/dev.json"
    val_img_data=f"{args.img_feat_dir}/twitter2017/train.pkl"
    ...
    if args.train == 1:
        ...
        train_dataset=MyDataset(args,train_data,train_img_data,emotion_data,relation_data,tokenizer)
        val_dataset=MyDataset(args,val_data,val_img_data,emotion_data,relation_data,tokenizer)
        test_dataset=MyDataset(args,test_data,test_img_data,emotion_data,relation_data,tokenizer)
        # Prepare optimizer
