...
class MyDataset(Data.Dataset):
    ...
    def transform_three(self,line,img_line):
        ...
        if semantic_relation_label==0 and emotional_relation_label==0:
            relation_s=0
            inputs=f"{instruction_irrelevant} Sentence: {text} Aspect: {aspect} Textual Clue: {text_clue}  OPTIONS: -positive -neutral -negative OUTPUT:"
        elif semantic_relation_label==1 and emotional_relation_label==0:
            relation_s=1
            inputs=f"{instruction_related} Sentence: {text} Aspect: {aspect} Textual Clue: {text_clue}  OPTIONS: -positive -neutral -negative OUTPUT:"
        else:
            relation_s=1
            inputs=f"{instruction_related} Sentence: {text} Aspect: {aspect} Textual Clue: {text_clue}  OPTIONS: -positive -neutral -negative OUTPUT:"
