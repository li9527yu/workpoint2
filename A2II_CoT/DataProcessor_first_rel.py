...
class MyDataset(Data.Dataset):
    ...
    #  二分类相关性
    def trasform_original(self,line,img_line):
        ...
        semantic_relation_label,emotional_relation_label=get_rel(relation)
        if semantic_relation_label==1 or emotional_relation_label==1:
            # 加入额外信息：Image Emotion 
            ...
            # 加入额外信息：Image Emotion Image Description
            inputs = f'{instruction_related} Description: {imagetext_meaning} Sentence: {text} Image Description: {image_caption} Image Emotion: {image_emotion} Aspect: {aspect} Aspect Description: {aspect} is {aspect_Context} OPTIONS: -positive -neutral -negative output:' 
            # 加入额外信息：Text Description Image Description
            # inputs = f'{instruction_related} Sentence: {text} Text Description: {text_description} Image Description: {image_caption} aspect: {aspect} OPTIONS: -positive -neutral -negative output:' 
            # 原始版本
    ...
    def trasform_noRelation(self,line,img_line):
        ...
        semantic_relation_label,emotional_relation_label=get_rel(relation)
        # if semantic_relation_label==1 or emotional_relation_label==1:
        #     # 加入额外信息：imagetext_meaning
        #     inputs = f'{instruction_related} Sentence: {text}  Aspect: {aspect} Image Emotion: {image_emotion} Description: {imagetext_meaning} OPTIONS: -positive -neutral -negative output:' 
            
        #     # 加入额外信息：Image Emotion Image Description
        #     # inputs = f'{instruction_related} Sentence: {text} Image Description: {image_caption} Image Emotion: {image_emotion} Aspect: {aspect} Aspect Description: {aspect} is {aspect_Context} OPTIONS: -positive -neutral -negative output:' 
        #     # 加入额外信息：Text Description Image Description
        #     # inputs = f'{instruction_related} Sentence: {text} Text Description: {text_description} Image Description: {image_caption} aspect: {aspect} OPTIONS: -positive -neutral -negative output:' 
        #     # 原始版本
        #     # inputs = f'{instruction_related} Sentence: {text}  aspect: {aspect} OPTIONS: -positive -neutral -negative output:' 
        # else:
        #     inputs = f'{instruction_irrelevant} Sentence: {text} Aspect: {aspect} Description: {imagetext_meaning}  OPTIONS: -positive -neutral -negative output:'
        #     # inputs = f'{instruction_irrelevant} Sentence: {text} Image Description: {image_caption} Image Emotion: {image_emotion} Aspect: {aspect} Aspect Description: {aspect} is {aspect_Context} OPTIONS: -positive -neutral -negative output:'
        #     # inputs = f'{instruction_irrelevant} Sentence: {text} Image Description: {image_caption} aspect: {aspect} OPTIONS: -positive -neutral -negative output:'

        relation_s=1
        inputs = f'{instruction_related} Sentence: {text}  Aspect: {aspect}  Description: {sentiment_meaning} OPTIONS: -positive -neutral -negative output:' 
        model_inputs = self.tokenizer(inputs, padding='max_length', truncation=True, max_length=max_input_len, return_tensors="pt")
        # 获取 tokenized 的 input_ids 和 attention_mask
        input_ids = model_inputs["input_ids"].squeeze(0)
        input_attention_mask = model_inputs["attention_mask"].squeeze(0)
    ...
    def transform_three(self,line,img_line):
        ...
        #     # irrelevant_instruction = f"Definition: Based solely on the information in the following sentence to identify the sentiment towards {aspect} in the text."
        #     # inputs= f'{instruction_irrelevant} Sentence: {text} Image Description: {image_caption}  aspect: {aspect} OPTIONS: -positive -neutral -negative. Output:'
        #     inputs= f'{instruction_related} Sentence: {text} Image Description: {image_caption} Sentence Emotion: {text_clue} Aspect: {aspect} OPTIONS: -positive -neutral -negative. Output:'
        # elif semantic_relation_label==1 and emotional_relation_label==0:
        #     relation_s=1
        #     inputs= f'{instruction_related} Sentence: {text} Image Description: {image_caption} Sentence Emotion: {text_clue}  Aspect: {aspect} OPTIONS: -positive -neutral -negative. Output:'
        # else:
        #     relation_s=1
        #     inputs= f'{instruction_related} Sentence: {text}  Image Description: {image_caption} Sentence Emotion: {text_clue} Image Emotion: {image_emotion}  Aspect: {aspect} OPTIONS: -positive -neutral -negative. Output:'
        ...
        if semantic_relation_label==0 and emotional_relation_label==0:
            ...
            relation_s=1
            inputs=f"{instruction_related} Sentence: {text} Sentence Emotion: {text_clue} Image Description: {image_caption} Aspect: {aspect}  OPTIONS: -positive -neutral -negative OUTPUT:"
        else:
            relation_s=1
        ...
        # return input_ids,input_multi_ids,input_attention_mask,input_multi_attention_mask,input_hidden_states,input_pooler_outputs,labels,output_labels
    ...
    # 只使用最新相关性
    def trasform_onlyRelation(self,line,img_line):
        ...
        semantic_relation_label,emotional_relation_label=get_rel(relation)
        if emotional_relation_label==0 and semantic_relation_label==0:
            # 原始版本
            inputs = f'{instruction_irrelevant} Sentence: {text}  Aspect: {aspect} OPTIONS: -positive -neutral -negative output:' 
        else:
            inputs = f'{instruction_related} Sentence: {text}  Aspect: {aspect}  OPTIONS: -positive -neutral -negative output:'
