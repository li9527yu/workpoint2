import torch.utils.data as Data
from torch.utils.data import dataset
from tqdm import tqdm
import pickle
import numpy as np
import os
import json

# 找到对应的相关性标签
def get_rel(relation):
    semantic_rel,emotional_rel=relation.split(',')
    semantic_relation_label,emotional_relation_label=0,0
    if 'irrelevant' in semantic_rel:
        semantic_relation_label=0
    else:
        semantic_relation_label=1

    if 'irrelevant' in emotional_rel:
        emotional_relation_label=0
    else:
        emotional_relation_label=1
   
    return semantic_relation_label,emotional_relation_label


# 数据集类
class MyDataset(Data.Dataset):
    def __init__(self,args,data_dir,img_data_dir,reason_data,caption_data,tokenizer):
        self.tokenizer=tokenizer
        self.max_seq_len=args.max_seq_len
        self.examples=self.creat_examples(data_dir)
        self.img_examples=self.creat_img_examples(img_data_dir)
        # 情感原因数据
        self.reason_examples=self.creat_examples(reason_data)
        self.caption_examples=self.creat_caption_examples(caption_data)
        self.number = len(self.examples)

    def __len__(self):
        return self.number
    def __getitem__(self,index):
        line=self.examples[index]
        img_line=self.img_examples[index]
        return self.transform(line,img_line)   

    def creat_examples(self,data_dir):
        with open(data_dir,"r") as f:
            dict=json.load(f)
        examples=[]
        for item in tqdm(dict,desc="CreatExample"):
            examples.append(item)
        return examples


    def creat_caption_examples(self,data_dir):
        with open(data_dir,"r") as f:
            dict=json.load(f)
 
        return dict
    

    def creat_img_examples(self,data_dir):
        with open(data_dir,"rb") as f:
            dict=pickle.load(f)
        examples=[]
        for item in tqdm(dict,desc="CreatExample"):
            examples.append(item)
        return examples

    def search_reason(self,aspect,img):
        for item in self.reason_examples:
            if aspect==item['aspect'] and img==item['image']:
                return item['response'] 
        return None
    
        # 找到对应的图像特征
    def search_img(self,aspect,img):
        for item in self.img_examples:
            item_img=item['image'].split('/')[-1]
            if item['aspect']==aspect and item_img==img:
                return item['hidden_state']
        return None
    
    def transform(self,line,img_line):
        max_input_len =self.max_seq_len
        input_hidden_states = img_line["hidden_state"]
        input_pooler_outputs = img_line["pooler_output"]
        value=line
        value=line
        text = value["text"]
        aspect = value["aspect"]
        text=text.replace('$T$',aspect)
        output_labels = value["sentiment"]
        # relation = value["relation_s"]
        # text_clue = value["text_clue"]
        # image_emotion = value["image_emtoion"]
        # image_caption = value["image_caption"]
        imgID=value['ImageID']
        imgID=imgID.split("/")[-1]

        input_hidden_states = self.search_img(aspect,imgID)
        input_pooler_outputs=0

        # 寻找对应的reason 数据
        reason=self.search_reason(aspect,imgID)
        caption=self.caption_examples[imgID][3]

        sentiment_map = {'0': 'negative', '1': 'neutral', '2': 'positive'}
        sentiment = sentiment_map[output_labels]
 
        inputs = "qea:  <image></image> caption: {} ".format(caption) + "sentence: {} aspect: {}.".format(text, aspect)
        # inputs = "qea: sentence: {} aspect: {}.".format(text, aspect)
        # output='<explain>{}</explain><emotion>{}</emotion>'.format(reason, sentiment)
        output='<emotion>{}</emotion><explain>{}</explain>'.format(sentiment,reason)
        model_inputs = self.tokenizer(inputs, padding='max_length', truncation=True, max_length=max_input_len, return_tensors="pt")
        input_ids = model_inputs["input_ids"].squeeze(0)
        input_attention_mask = model_inputs["attention_mask"].squeeze(0)

        model_inputs_labels = self.tokenizer(output, padding='max_length', truncation=True, max_length=128, return_tensors="pt")
        model_inputs_labels['input_ids'][model_inputs_labels['input_ids'] == self.tokenizer.pad_token_id] = -100
        labels = model_inputs_labels["input_ids"].squeeze(0)

        return input_ids,input_attention_mask,input_hidden_states,input_pooler_outputs,labels,int(output_labels)
