...
class MyFlanT5(nn.Module):
    def __init__(self,args,model,tokenizer,model_name='/public/home/ghfu/lzy/model/instructblip-flan-t5-xl'):
        super().__init__()
        ...
        self.ea_generation_config = GenerationConfig.from_pretrained(args.model_path, 'ea_generation_config.json')
    ...
    def forward(self,input_ids,input_attention_mask,input_hidden_states,decoder_input_ids=None, decoder_attention_mask=None, labels=None):
        ...
        inputs_embeds = torch.cat([inputs_embeds[:, :2, :], query, inputs_embeds[:, 2:, :]], dim=1)
        # [bs,160(32+128),768]
    ...
    def generate(self,input_ids,attention_mask,input_hidden_states,decoder_input_ids=None, decoder_attention_mask=None, labels=None):
        query=self.language_projection(input_hidden_states)
        # query=input_hidden_state
        query_attention_mask = torch.ones(
            query.size()[:-1], dtype=torch.long, device=query.device
        ...
        # decoder_input_ids = torch.tensor([[0, 32102]], device=inputs_embeds.device)
