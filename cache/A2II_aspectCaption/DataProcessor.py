...
class MyDataset(Data.Dataset):
    def __init__(self,data_dir,caption_data,tokenizer,max_seq_len=64):
        self.tokenizer=tokenizer
        self.max_seq_len=max_seq_len
        self.examples=self.creat_examples(data_dir)
        self.caption_data=self.creat_captions(caption_data)
    ...
    def __getitem__(self,index):
        ...
        caption=self.caption_data[index]
        return self.transform(line,caption)   
    ...
    def creat_captions(self,data_dir):
        with open(data_dir,"rb") as f:
            dict=pickle.load(f)
        examples=[]
        for item in tqdm(dict,desc="CreatExample"):
            examples.append(item)
        return examples
    def transform(self,line,caption):
        max_input_len =self.max_seq_len
        value=line
        caption_data=caption['descriptions']
        ...
        input_re = f'{instruction_related} Sentence: {input_texts} Aspect: {input_aspects} Aspect Context:{caption_data} OPTIONS: -positive -neutral -negative output:' 
        input_ir = f'{instruction_irrelevant} Sentence: {input_texts} Aspect: {input_aspects} Aspect Context:{caption_data} OPTIONS: -positive -neutral -negative output:'
