...
def get_parser():
    ...
    parser.add_argument('--output_dir', default="/data/lzy1211/code/A2II/instructBLIP/results//twitter2015-our-tiaocan/", type=str)
...
def post_dataloader(batch):
    device = torch.device('cuda:7' if torch.cuda.is_available() else 'cpu')
    input_ids,input_attention_mask,input_hidden_states,input_pooler_outputs,labels,label_index=batch
...
def evaluate_on_test(model,test_dataloader,tokenizer,eos_token_id, logger):
    ...
    for batch in tqdm(test_dataloader, desc="Testing"):
        input_ids,input_attention_mask,input_hidden_states,input_pooler_outputs,labels= post_dataloader(batch)
        with torch.no_grad():
...
def main(args):
    ...
    # sentiment数据
    # img_dir=f"/public/home/ghfu/lzy/code/instructBLIP/img_feat/{args.dataset}.pkl"
    # img_feat={}
    # try:
    #     with open(img_dir, 'rb') as f:  # 以二进制模式打开文件
    #         img_feat = pickle.load(f)  # 反序列化
    ...
    for train_idx in trange(int(args.EPOCHS), desc="Epoch"):
        ...
        for step,data in enumerate(tqdm(train_dataloader,desc="Iteration")):
            batch=data
            # Sentiment
            input_ids,input_attention_mask,input_hidden_states,input_pooler_outputs,labels,label_index= post_dataloader(batch)
            outputs= model(input_ids=input_ids,
                            input_attention_mask=input_attention_mask,              
                            input_hidden_states    = input_hidden_states,
                            labels=labels)
        ...
        #     }   
        pred_sequence,senti_labels=[],[]
        for batch in tqdm(val_dataloader, desc="Testing"):
            input_ids,input_attention_mask,input_hidden_states,input_pooler_outputs,labels,senti_label= post_dataloader(batch)
            with torch.no_grad():
                outputs= model.generate(input_ids=input_ids,
                                attention_mask=input_attention_mask,              
        ...
        senti_result = compute_metrics(senti_preds, senti_labels)
        result = {     
            f'Dev_senti_acc':senti_result['acc'],    
            f'senti_f1':senti_result['f1'],  
        ...
        #         # 计算准确率
        #         test_senti_acc += sum([1 for pred, true in zip(predicted_labels, true_labels) if pred == true])
        #         test_sa_examples += labels.size(0)

                
        # test_senti_acc=test_senti_acc/test_sa_examples
        # result = {
        #     'test_sa_examples':test_sa_examples,         
        #     'test_senti_acc':test_senti_acc,             
        #     }        
        # wandb.log(
        # {
        ...
        test_pred_sequence,test_senti_labels=[],[]
        for batch in tqdm(test_dataloader, desc="Testing"):
            input_ids,input_attention_mask,input_hidden_states,input_pooler_outputs,labels,senti_label= post_dataloader(batch)
            with torch.no_grad():
                outputs= model.generate(input_ids=input_ids,
                                ...
                                input_hidden_states    = input_hidden_states,
                                labels=labels)
                test_pred_sequence.extend(outputs)
                test_senti_labels.extend(senti_label.detach().cpu().numpy())

        test_senti_preds=parse_sequences(test_pred_sequence)
        test_senti_result = compute_metrics(test_senti_preds, test_senti_labels)
        result = {     
            f'test_senti_acc':test_senti_result['acc'],    
            f'test_senti_f1':test_senti_result['f1'],  
        ...
        # 保存 模型
        # save model
        if senti_result['acc'] >= max_senti_acc:
    ...
    evaluate(model,test_dataloader, logger)
