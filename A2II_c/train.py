...
def post_dataloader(batch):
    ...
    input_re_ids,input_re_attention_mask,input_ir_ids,input_ir_attention_mask,input_hidden_states,input_pooler_outputs,img_feat,labels=batch    
    ...
    img_feat=img_feat.squeeze(1).clone().detach().to(input_re_ids.device)
            
    return input_re_ids,input_re_attention_mask,input_ir_ids,input_ir_attention_mask,input_hidden_states,input_pooler_outputs,img_feat,labels
...
def evaluate_on_test(model,test_dataloader,tokenizer,eos_token_id, logger):
    ...
    for batch in tqdm(test_dataloader, desc="Testing"):
        input_re_ids,input_re_attention_mask,input_ir_ids,input_ir_attention_mask,input_hidden_states,input_pooler_outputs,img_feat,labels= post_dataloader(batch)
        with torch.no_grad():
...
def main(args):
    ...
    train_dataset=MyDataset(train_data,train_img_data,tokenizer, max_seq_len= args.max_seq_length)
    val_dataset=MyDataset(val_data,val_img_data,tokenizer, max_seq_len= args.max_seq_length)
    test_dataset=MyDataset(test_data,test_img_data,tokenizer, max_seq_len= args.max_seq_length)
