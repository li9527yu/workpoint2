...
from DataProcessor_test_relC import MyDataset
...
if __name__ == "__main__":
    ...
    qformer_rel=[]
    try:
        with open('/public/home/ghfu/lzy/code/instructBLIP/A2II_first_work/analysis/twitter_rel_pred/qformer_pred.pkl', 'rb') as f:  # 以二进制模式打开文件
            qformer_rel = pickle.load(f)  # 反序列化
    except Exception as e:
        print(f"读取 .pkl 文件时出错: {e}")
    # 加载数据
    test_data=f"{data_dir}/{dataset}/test.pkl"
    test_dataset=MyDataset(test_data,tokenizer,Roberta_tokenizer,img_feat,qformer_rel, max_seq_len= max_seq_length)
