...
def contextual_fusion(prob_ori, prob_context, alpha=0.3, beta=0.4):
    """
    融合策略：WisdoM
    :param prob_ori: numpy array, 原始预测概率
    :param prob_context: numpy array, 上下文预测概率
    :param alpha: float, 阈值，控制是否直接用 prob_ori
    :param beta: float, 插值系数
    """
...
# logit_generate_batch
def evaluate_logit(model, test_dataloader, logger):
    model.eval()
    ...
    for batch in tqdm(test_dataloader, desc="Testing"):
        (
            ...
            labels, senti_label, relation_label
        ) = post_dataloader(batch)

        with torch.no_grad():
            # 原始预测
            results = model.logit_generate_batch(
                input_ids=input_ids,
            ...
            # 加入上下文的预测
            multi_results = model.logit_generate_batch(
                input_ids=input_multi_ids,
        ...
        # ===== 融合结果 =====
        for r_ori, r_ctx in zip(results, multi_results):
            prob_ori = np.array(r_ori["logit_score"])
            prob_ctx = np.array(r_ctx["logit_score"])
            prob_final = contextual_fusion(prob_ori, prob_ctx, alpha=0.3, beta=0.4)
            chosen_idx = np.argmax(prob_final)
            pred_sequence.append(option_keys[chosen_idx])
...
def train(args,model,train_dataset,val_dataset,optimizer_t5,logger,):
    ...
    for train_idx in trange(int(args.EPOCHS), desc="Epoch"):
        ...
        for step,data in enumerate(tqdm(train_dataloader,desc="Iteration")):
            ...
            # Sentiment
            input_ids,input_multi_ids,input_attention_mask,input_multi_attention_mask,input_hidden_states,input_pooler_outputs,labels,senti_label,relation_label= post_dataloader(batch)
            outputs= model(input_ids=input_ids,attention_mask=input_attention_mask,          
                            input_hidden_states    = input_hidden_states,
                            labels=labels,
                            relation=relation_label
                        )
        ...
        logger.info("  Batch size = %d", args.BATCH_SIZE)
        dev_result=evaluate_logit(model,val_dataloader, logger)
...
def main(args):
    ...
    test_results=evaluate_logit(model,test_dataloader,logger)
