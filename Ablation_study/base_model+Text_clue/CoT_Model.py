...
class MyFlanT5(nn.Module):
    def __init__(self,model_path,tokenizer,model_name='/public/home/ghfu/lzy/model/instructblip-flan-t5-xl'):
        ...
        # 在输入部分添加 MLP 层
        # 自动获取 T5 的 hidden_size
        self.hidden_size = self.language_model.config.d_model  # base=768, large=1024, xl=2048
        
        # 将外部输入的 hidden_states 投影到与 T5 一致的维度
        self.language_projection = nn.Linear(768, self.hidden_size)
        # self.selector=nn.Linear(768, 2)
        self.init_linear_weight()

        # ---- Dropout 层 ----
        # 投影后的dropout（防止query过拟合）
        # self.proj_dropout = nn.Dropout(0.3)
        # # 拼接后dropout（防止embedding过拟合）
