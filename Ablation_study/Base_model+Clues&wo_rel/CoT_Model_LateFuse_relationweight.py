...
class MyFlanT5(nn.Module):
    ...
    def get_weight_from_relation(self, relation):
            """
            根据 relation 决定融合时图像权重
            relation: 0=无关, 1=情感相关, 2=语义相关但情感无关
            返回一个 batch 的 weight 向量
            """
            if isinstance(relation, int):
                # 单个值
                mapping = {0: 0.2, 1: 0.5, 2: 0.5}
                return mapping.get(relation, 0.5)
            else:
                # batch tensor
                weight = torch.zeros_like(relation, dtype=torch.float) 
                weight[relation == 0] = 0.2
                weight[relation == 1] = 0.5
                weight[relation == 2] = 0.5
                return weight
    ...
    def forward(self,input_ids,input_multi_ids,attention_mask,input_multi_attention_mask,input_hidden_states,relation,decoder_input_ids=None, decoder_attention_mask=None, labels=None):
        ...
        # === 融合 ===
        weight = self.get_weight_from_relation(relation)  # batch 权重
        # broadcast [batch, 1, 1] for matching logits shape
        if isinstance(weight, torch.Tensor):
            weight = weight.view(-1, 1, 1)
        fused_logits = (1 - weight) * outputs.logits + weight * multi_outputs.logits
    ...
    def generate(self,input_ids,input_multi_ids,attention_mask,input_multi_attention_mask,input_hidden_states,relation,decoder_input_ids=None, decoder_attention_mask=None, labels=None):
        ...
        # weight=0.5
        # === 融合 ===
        weight = self.get_weight_from_relation(relation)
        if isinstance(weight, torch.Tensor):
            weight = weight.view(-1, 1)  # [batch, 1]

        fused_token_ids = []
        for step_logits_single, step_logits_multi in zip(outputs.scores, multi_outputs.scores):
            fused_logits = (1 - weight) * step_logits_single + weight * step_logits_multi
